...
export function ReviewDialog({ open, document, onChange, onClose }: { open: boolean; document: EditorDocument; onChange: (document: EditorDocument) => void; onClose: () => void }) {
  ...
  if (!open) return null;
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="dialog review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-title">
    ...
    <div className="dialog-tabs"><button className={tab === 'find' ? 'selected' : ''} type="button" onClick={() => setTab('find')}>찾기·바꾸기</button><button className={tab === 'spell' ? 'selected' : ''} type="button" onClick={() => setTab('spell')}>한국어 기본 교정</button></div>
