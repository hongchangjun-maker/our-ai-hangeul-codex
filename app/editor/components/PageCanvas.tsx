...
function RichNode({ node, nodeKey }: { node: unknown; nodeKey: string | number }) {
  ...
  const blockStyle = {
    ...
    marginLeft: value.attrs?.marginLeftPx === undefined ? undefined : `${value.attrs.marginLeftPx}px`,
    marginRight: value.attrs?.marginRightPx === undefined ? undefined : `${value.attrs.marginRightPx}px`,
    textIndent: value.attrs?.textIndentPx === undefined ? undefined : `${value.attrs.textIndentPx}px`,
  };
    ...
    <div className={fileDragActive ? 'canvas-area file-drag-active' : 'canvas-area'} ref={canvasRef} onDragOver={(event) => { event.preventDefault(); setFileDragActive(true); }} onDragLeave={(event) => { if (event.currentTarget === event.target) setFileDragActive(false); }} onDrop={() => setFileDragActive(false)}>
      ...
      <div className={`page-stack ${pageViewMode === 'spread' ? 'spread' : 'single'}`}>
        {document.pages.map((page, index) => {
          ...
          <span className="page-sheet-label" aria-hidden="true">{index + 1}쪽 · {page.preset}{page.customSizeMm ? ' 원본' : ''} · {geometry.widthMm}×{geometry.heightMm}mm</span>
          <article className="paper exportable-page" data-page-index={index} data-page-width-mm={geometry.widthMm} data-page-height-mm={geometry.heightMm} aria-label={`${page.preset} 문서 ${index + 1}쪽, ${geometry.widthMm}×${geometry.heightMm}mm`} aria-current={index === currentPage ? 'page' : undefined} style={{ width: geometry.widthPx, height: geometry.heightPx, transform: `scale(${pageScale})`, background: page.background }} onClick={(event) => activatePage(index, { left: event.clientX, top: event.clientY })} onDrop={(event) => { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); onFiles(event.dataTransfer.files, index, { x: (event.clientX - rect.left) / pageScale, y: (event.clientY - rect.top) / pageScale }); }}>
            ...
            <span className={`active-frame-size ${page.guideStyle === 'corners' ? 'corner-guides' : ''}`} style={{ '--inverse-page-scale': String(1 / pageScale) } as React.CSSProperties}>
            {showGuides ? <>
                {index === currentPage ? <>
