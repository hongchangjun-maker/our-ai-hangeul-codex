...
export function FontLibraryDialog({ open, favoriteFonts, onClose, onToggleFavorite, onApply }: {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="dialog font-library-dialog" role="dialog" aria-modal="true" aria-labelledby="font-library-title">
      ...
      <div className="font-library-controls">
        <div role="tablist" aria-label="글꼴 분류"><button className={script === 'all' ? 'selected' : ''} type="button" onClick={() => setScript('all')}>전체</button><button className={script === 'korean' ? 'selected' : ''} type="button" onClick={() => setScript('korean')}>한글</button><button className={script === 'english' ? 'selected' : ''} type="button" onClick={() => setScript('english')}>English</button></div>
